        ...
        <div v-if="viewType === 'grid'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div v-for="student in students" :key="student.id" class="bg-white rounded-2xl shadow p-6 flex flex-col items-center w-full max-w-xs mx-auto">
            ...
            <div class="flex gap-4 mt-2">
              <button @click="openModal('edit', student)" class="w-10 h-10 flex items-center justify-center rounded-full bg-yellow-100 hover:bg-yellow-200 text-yellow-600 shadow transition">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487a2.1 2.1 0 1 1 2.97 2.97L7.5 19.788l-4 1 1-4 13.362-13.3z" />
                </svg>
              </button>
              <button @click="openModal('delete', student)" class="w-10 h-10 flex items-center justify-center rounded-full bg-red-100 hover:bg-red-200 text-red-600 shadow transition">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
              <button @click="openModal('details', student)" class="w-10 h-10 flex items-center justify-center rounded-full bg-green-100 hover:bg-green-200 text-green-600 shadow transition">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
const loadStudents = async () => {
  ...
  try {
    ...
    students.value = response.data.map(student => ({ ...student, type: 'Student' }));
  } catch (error) {
...
const handleModalAction = async (action) => {
  ...
  if (action.action == 'create') {
    try {
      await axios.post('/api/students', action.data);
      loadStudents();
    } catch (error) {
    ...
    try {
      await axios.put(`/api/students/${action.data.id}`, action.data);
      loadStudents();
    } catch (error) {
    ...
    try {
      await axios.delete(`/api/students/${action.id}`);
      loadStudents();
    } catch (error) {
